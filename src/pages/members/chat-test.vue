  ...
  <div class="mui-content" v-wechat-title="this.detailtitle">
    <div class="msg-content" :style="{'height': msgHeight}" @scroll.passive="loadMore($event)">
      ...
      <div class="comment-chat-display" v-if="(order.status == 4 || status == 5) && order.mid == mid">
        <p class="comment-msg"><divider>服务已结束，如老师的服务有帮助到您，请感谢：</divider></p>
...
<script>
  ...
  export default {
    ...
    mounted () {
      this.sendInfo = {'openid': cookie.getCookie('openid'), 'avatar': sessionStorage.getItem("avatar")}
      let token = cookie.getCookie('token')
      if (token === null) {
        sessionStorage.setItem("url", this.$route.path);
        this.getWechatAuth()
      ...
      // this.$router.go(0) 只刷新一次，通过这种方法重新连接一次融云
      let time = dateFormat(new Date(), 'YYYYMMDDHHmmss');
      if( this.$route.query.time + 0.01  > time){
        this.$router.go(0)
      }
      // this.$router.go(0) 只刷新一次
      this.$nextTick(function () {
        document.getElementById("msg-rongyun-content").scrollIntoView(false);
      })
    ...
    methods: {
      ...
      getOrderInfo: function () {
        return this.$api.orderdetail(this.$route.query.orderid).then(res => {
          ...
          // 得到我自己的信息，传入一个out_order
          this.getUserInfo(mid,0)
          //得到对方信息
      ...
      getUserInfo: function (id,type) {
        return this.$api.userInfo(id).then(res => {
          if(type == 1){
            ...
            sessionStorage.setItem("targetId", this.$route.query.orderid + "_" + res.data.openid)
          }
        }).catch(err => {
