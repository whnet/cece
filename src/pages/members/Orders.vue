      ...
      <div class="ui-tab-content" v-if="list.length > 0">
        <div class="ui-cells" v-for="(item, i) in list">
          ...
          <div class="ui-cell list-bottom" v-if="item.status == 1">
            <div class="ui-cell-primary">
              <a class="go-evaluate wait" v-if="item.kaiqi == 1 && item.status == 1" @click="goChat(item.id)">等待接单</a>
...
<script>
  import { Tab, TabItem, Search, Scroller, dateFormat, XHeader, Divider, FormPreview,
  ...
  export default {
    ...
    mounted (){
      ...
      this.msgContent = (window.msgContent - footer - 50) + 'px'
      window.onresize = () => {
        return (() => {
          window.msgContent = document.documentElement.clientHeight
          this.msgContent = (window.msgContent - footer - 50) + 'px'
    ...
    methods: {
      ...
      goChat(id){
        ...
        this.$router.push({path:'/chat', query: {orderid: id,time: time}})
        return;
      },
